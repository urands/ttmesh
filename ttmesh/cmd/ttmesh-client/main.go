...
func main() {
    ...
    logger, _ := zap.NewDevelopment(); zap.ReplaceGlobals(logger); defer logger.Sync()
    ...
    if err != nil { fatalf("marshal hello env: %v", err) }
    if err := st.SendBytes(heB); err != nil { fatalf("send hello: %v", err) }
    fmt.Println("Hello sent; peerID:", peerID)

    // Optionally receive HelloAck (best-effort)
    if ackB, err := st.RecvBytes(); err == nil {
        var ackEnv ttmeshproto.Envelope
        if proto.Unmarshal(ackB, &ackEnv) == nil {
    ...
    if *dest != "" { te.Header.Dest = &ttmeshproto.NodeRef{Id: *dest} }
    teB, err := proto.Marshal(te)
    if err != nil { fatalf("marshal test env: %v", err) }
    if err := st.SendBytes(teB); err != nil { fatalf("send test: %v", err) }
